...
void loop() {
  ...
  if (currentTime - lastPrintTime >= 300) {
    ...
    float pressureDifference = finalAdcAverage_OU2 - ADCoffset;
    float presureReal = (10.0/700)*pressureDifference;  // Линейна зависимост
    float airSpeed=0;

    // Визуализиране на стойностите на дисплея
    tft.setCursor(indentText, 10);//Ред 1
    tft.print("Vcc: ");
    tft.fillRect(indentText+5*6*sizeText, 10, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    tft.setCursor(indentText+5*6*sizeText, 10);
    tft.print(finalVccAverage);
    //tft.print(" units");

    tft.setCursor(indentText, 10+1*lineSpacing);//Ред2
    tft.print("Vsensor: ");
    tft.fillRect(indentText+9*6*sizeText, 10+1*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    ...
    tft.print("Diff: ");
    tft.fillRect(indentText+6*6*sizeText, 10+2*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    ...
    tft.print("ADCoffset: ");
    tft.fillRect(indentText+11*6*sizeText, 10+3*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    tft.setCursor(indentText+11*6*sizeText, 10+3*lineSpacing);
    tft.print(ADCoffset);

    tft.setCursor(indentText, 10+4*lineSpacing);//Ред 5
    tft.print("ADC: ");
    tft.fillRect(indentText+5*6*sizeText, 10+4*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    ...
    tft.print("Presure = ");
    tft.fillRect(indentText+10*6*sizeText, 10+5*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    tft.setCursor(indentText+10*6*sizeText, 10+5*lineSpacing);
    tft.print(presureReal, 2);
    tft.print(" mmH2O");

    tft.setCursor(indentText, 10+6*lineSpacing);//Ред 7
    tft.print("Vair = ");
    tft.fillRect(indentText+7*6*sizeText, 10+6*lineSpacing, 120, 28, TFT_BLUE);  // Изчистване на предишното показание
    tft.setCursor(indentText+7*6*sizeText, 10+6*lineSpacing);
